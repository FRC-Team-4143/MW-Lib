...
ProxyOdomMsg loadOdom(nav_msgs::msg::Odometry::ConstSharedPtr msg) {
    ...
    proxy_msg.x_pos = static_cast<int32_t>(msg->pose.pose.position.x * FIXED_PT_DIV);
    proxy_msg.y_pos = static_cast<int32_t>(msg->pose.pose.position.y * FIXED_PT_DIV);
    proxy_msg.theta_pos = static_cast<int32_t>(yaw * FIXED_PT_DIV);

    // convert the variance data
    proxy_msg.x_var = static_cast<int32_t>(msg->pose.covariance.at(0) * FIXED_PT_DIV);
    proxy_msg.y_var = static_cast<int32_t>(msg->pose.covariance.at(7) * FIXED_PT_DIV);
    proxy_msg.theta_var = static_cast<int32_t>(msg->pose.covariance.at(14) * FIXED_PT_DIV);

    proxy_msg.x_dot = static_cast<int32_t>(msg->twist.twist.linear.x * FIXED_PT_DIV);
    proxy_msg.y_dot = static_cast<int32_t>(msg->twist.twist.linear.y * FIXED_PT_DIV);
    proxy_msg.theta_dot = static_cast<int32_t>(msg->twist.twist.angular.z * FIXED_PT_DIV);
...
ProxyModuleStatesMsg loadModules(chassis_msgs::msg::ModuleStates::ConstSharedPtr msg) {
    ...
    // load module 1
    proxy_msg.ang_pos_1 = static_cast<int32_t>(msg->observed_states.at(0).angle_rad * FIXED_PT_DIV);
    proxy_msg.lin_vel_1 = static_cast<int32_t>(msg->observed_states.at(0).lin_vel * FIXED_PT_DIV);

    // load module 2
    proxy_msg.ang_pos_2 = static_cast<int32_t>(msg->observed_states.at(1).angle_rad * FIXED_PT_DIV);
    proxy_msg.lin_vel_2 = static_cast<int32_t>(msg->observed_states.at(1).lin_vel * FIXED_PT_DIV);

    // load module 3
    proxy_msg.ang_pos_3 = static_cast<int32_t>(msg->observed_states.at(2).angle_rad * FIXED_PT_DIV);
    proxy_msg.lin_vel_3 = static_cast<int32_t>(msg->observed_states.at(2).lin_vel * FIXED_PT_DIV);

    // load module 4
    proxy_msg.ang_pos_4 = static_cast<int32_t>(msg->observed_states.at(3).angle_rad * FIXED_PT_DIV);
    proxy_msg.lin_vel_4 = static_cast<int32_t>(msg->observed_states.at(3).lin_vel * FIXED_PT_DIV);
...
ProxyVisionDetection loadDetection(const std_msgs::msg::Header& hdr, const vision_msgs::msg::Detection2D& msg,
    ...
    proxy_msg.theta_x = static_cast<int64_t>(theta_x * 10e6);
    proxy_msg.theta_y = static_cast<int64_t>(theta_y * 10e6);
