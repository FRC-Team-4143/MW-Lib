...
public class NovaArmMech extends NovaMechBase {
    ...
    public void writeOutputs(double timestamp) {
        ...
        switch (control_mode_) {
            case POSITION:
                ...
                motors_[0].setPosition(motor_position_rot);
                break;
            case VELOCITY:
    ...
    public void logData() {
        ...
        DogLog.log(getLoggingKey() + "control/duty_cycle/actual", duty_cycle_target_);

        // per motor data
        for (int i = 0; i < motors_.length; i++) {
            DogLog.log(getLoggingKey() + "motor" + i + "/current_draw", current_draw_[i]);
            DogLog.log(getLoggingKey() + "motor" + i + "/temp", motor_temp_c_[i]);
        }
    }
